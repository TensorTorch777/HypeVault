#!/usr/bin/env python3
"""Regenerate training curves and validation dashboards from saved artifacts.

Examples
--------
  # Training grid only (no GPU; uses ml/checkpoints/history.json)
  python regenerate_plots.py

  # Custom paths + subtitle (matches report figures)
  python regenerate_plots.py \\
    --history /path/to/history.json \\
    --out_dir /path/to/out \\
    --subtitle "Stage 2"

  # Confusion / ROC / PR dashboard (needs dataset + checkpoint; GPU recommended)
  python regenerate_plots.py --eval \\
    --data_root /path/to/Label_*_dataset_root \\
...
def _val_epoch_cpu_safe(
    ...
    """Validation pass: uses train.val_epoch on CUDA; plain fp32 on CPU."""
    if device.type == "cuda":
...
def run_eval_plots(
    ...
    if device.type == "cpu":
        ...
        # DINOv2-Giant @ 518px is VRAM-heavy; tune caps by GPU RAM.
...
def main() -> None:
    p = argparse.ArgumentParser(description="Regenerate HypeVault training / eval plots")
    here = Path(__file__).resolve().parent
    p.add_argument(
        "--history",
        type=Path,
        default=here / "checkpoints" / "history.json",
        help="history.json from a training run",
    )
    p.add_argument(
        "--out_dir",
        type=Path,
    ...
    p.add_argument(
        ...
        type=str,
        default=None,
        help='Optional third line under title (e.g. "Stage 2")',
    )
    p.add_argument(
        "--eval",
        action="store_true",
        help="Run validation and write confusion_matrix_eval.png (needs --data_root)",
    )
    p.add_argument("--data_root", type=Path, default=None, help="Dataset root with Label_* folders")
    p.add_argument(
        "--checkpoint",
        type=Path,
        default=None,
        help="best_model.pt (default: <out_dir>/best_model.pt)",
    )
    args = p.parse_args()
    ...
    if args.eval:
        ...
        root = args.data_root
        if root is None:
            raise SystemExit("--eval requires --data_root pointing at your Label_* dataset tree")
        if not root.is_dir():
